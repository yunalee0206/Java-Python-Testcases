...
class BaseSetGeneratorTest {
    ...
    void testRandZeroTests() {
        BaseSetGenerator generator = new BaseSetGenerator(oneArgSimple, 0);
        assertEquals(0, generator.genRandTests(Set.of()).size());
    }
    ...
    void testRandNumTestsNonZero() {
        ...
        assertEquals(3, generator.genRandTests(Set.of()).size());
    }
    ...
    void testRandOneArgOneOption() {
        ...
        assertEquals(oneArgOneOptionRandVals, generator.genRandTests(Set.of()));
    }
    ...
    void testRandMultipleArgsOneOptionEach() {
        ...
        assertEquals(multipleArgsOneOptionRandVals, generator.genRandTests(Set.of()));
    }
    ...
    private static void setUpOneArgNested() {
        ...
        listNode.setExDomain(List.of(0, 1, 2));
        listNode.setRanDomain(List.of(0, 1, 2));
    ...
    private static void setUpMultipleArgsOneOption() {
        ...
        multipleArgsOneOption = List.of(node1, node2);

        // Populate the expected result of exhaustive generation
        List<APyObj<?>> exArgs = List.of(new PyIntObj(17), new PyTupleObj<>(
                List.of(new PyIntObj(3), new PyIntObj(3))));
        multipleArgsOneOptionExVals = Set.of(new TestCase(exArgs));

        // Populate the exhaustive set of values from the random domain
        List<APyObj<?>> randArgs = List.of(new PyIntObj(19), new PyTupleObj<>(
                List.of(new PyIntObj(5), new PyIntObj(5), new PyIntObj(5),
                        new PyIntObj(5))));
    ...
    private static void setUpMultipleArgsSimple() {
        ...
        firstArgSimple.setExDomain(List.of(888, 88, 8));
        firstArgSimple.setRanDomain(List.of(7777, 777, 77, 7));

        // Second arg: a float
        PyFloatNode secondArgSimple = new PyFloatNode();
        secondArgSimple.setExDomain(List.of(8.88, 8.8, 8.0));
        secondArgSimple.setRanDomain(List.of(7.77, 7.7, 7.0));

        // Third arg: a bool
        PyBoolNode thirdArgSimple = new PyBoolNode();
        thirdArgSimple.setExDomain(List.of(0, 1));
        thirdArgSimple.setRanDomain(List.of(0, 1));

        // Wrap all args into a list
        multipleArgsSimple =
                List.of(firstArgSimple, secondArgSimple, thirdArgSimple);
        ...
        for (Number i : firstArgSimple.getExDomain()) {
            for (Number j : secondArgSimple.getExDomain()) {
                for (boolean b : new boolean[]{true, false}) {
                    multipleArgsSimpleExVals.add(new TestCase(List.of(
        ...
        for (Number i : firstArgSimple.getRanDomain()) {
            for (Number j : secondArgSimple.getRanDomain()) {
                for (boolean b : new boolean[]{true, false}) {
                    multipleArgsSimpleRandVals.add(new TestCase(List.of(
                            new PyIntObj(i.intValue()), new PyFloatObj(j.doubleValue()),
                            new PyBoolObj(b))));
                }
            }
    ...
    private static void setUpMultipleArgsNested() {
        // First arg: a string
        PyStringNode firstArgNested = new PyStringNode(Set.of('r', 'j', 's', '7'));
        ...
        secondArgInner.setExDomain(List.of(0, 1));
        secondArgInner.setRanDomain(List.of(0, 1));
        PySetNode<PyBoolObj> secondArgNested = new PySetNode<>(secondArgInner);
        secondArgNested.setExDomain(List.of(0, 1));
        secondArgNested.setRanDomain(List.of(1, 2));
        ...
        keyNode.setExDomain(List.of(10, 20, 30));
        keyNode.setRanDomain(List.of(100, 200, 300, 400));
        ...
        thirdArgNested.setExDomain(List.of(1, 3));
        thirdArgNested.setRanDomain(List.of(2, 4));
        ...
        for (APyObj<?> o1 : firstArgExOptions) {
            for (APyObj<?> o2 : secondArgExOptions) {
                for (APyObj<?> o3 : thirdArgExOptions) {
                    multipleArgsNestedExVals.add(new TestCase(List.of(o1, o2, o3)));
                }
            }
        }
        ...
        for (APyObj<?> o1 : firstArgRandOptions) {
            for (APyObj<?> o2 : secondArgRandOptions) {
                for (APyObj<?> o3 : thirdArgRandOptions) {
                    multipleArgsNestedRandVals
                            .add(new TestCase(List.of(o1, o2, o3)));
                }
            }
        }

        multipleArgsNested =
                List.of(firstArgNested, secondArgNested, thirdArgNested);
    }
    ...
    private static boolean runTrialsRand(List<APyNode<?>> nodes, Set<TestCase> validTests,
        ...
        for (int i = 0; i < numTrials; i++) {
            Set<TestCase> tests = new BaseSetGenerator(nodes, numTests).genRandTests(Set.of());
            results.add(tests);
