...
class MainTest {
    ...
    void testMultipleCasesComplex() {
        ...
        PyListObj<PyIntObj> list2 =
                new PyListObj<>(List.of(new PyIntObj(2), new PyIntObj(2)));
        ...
        for (PyListObj<?> list : new PyListObj[]{list1, list2}) {
            for (PyTupleObj<?> tup : new PyTupleObj[]{tup1, tup2}) {
                ...
                option.add(new TestCase(List.of(set1, list1, tup1)));
                option.add(new TestCase(List.of(set2, list, tup)));
    ...
    private static String[] buildArgs(String funcName, String configName, String implDir) {
        ...
        // Build solution path
        String solutionPath = userDir + "/src/test/rice/test/pyfiles/sols/" + funcName + "sol.py";

        // Build implDir path
        String implDirPath = userDir + "/src/test/rice/test/pyfiles/" + implDir;
        return new String[]{configFilePath, solutionPath, implDirPath};
    }
