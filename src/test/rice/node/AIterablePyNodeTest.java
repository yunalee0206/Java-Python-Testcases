 ...
 */
class AIterablePyNodeTest extends APyNodeTest {
    ...
    private static void setUpFewOptions() {
        ...
        twoLenOneChild.setExDomain(List.of(0, 1));
        twoLenOneChild.setRanDomain(List.of(0, 1));
        ...
        fourLenTwoChild.setExDomain(List.of(0, 1));
        fourLenTwoChild.setRanDomain(List.of(0, 1));
    ...
    private static Set<PyListObj<PyBoolObj>> genExpectedFourLenTwo() {
        ...
        for (boolean val1 : List.of(true, false)) {
            for (boolean val2 : List.of(true, false)) {
                List<PyBoolObj> val = List.of(new PyBoolObj(val1),
                        new PyBoolObj(val2));
    ...
    private static void setUpManyLenTwo() {
        ...
        manyLenTwoChild.setExDomain(List.of(2, 0, 1));
        manyLenTwoChild.setRanDomain(List.of(2, 0, 1));
    ...
    private static Set<PyListObj<PyIntObj>> genExpectedManyLenTwo() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                List<PyIntObj> val = List.of(new PyIntObj(i), new PyIntObj(j));
                expected.add(new PyListObj<>(val));
            }
        }
        return expected;
    ...
    private static void setUpLensZeroToOne() {
        ...
        lenZeroToOneChild.setExDomain(List.of(3, 5, 4));
        lenZeroToOneChild.setRanDomain(List.of(6, 7));

        lensZeroToOne = new PyListNode<>(lenZeroToOneChild);
        lensZeroToOne.setExDomain(List.of(0, 1));
        lensZeroToOne.setRanDomain(List.of(0, 1));
    ...
    private static void setUpLensZeroToThree() {
        ...
        lenZeroToThreeChild.setExDomain(List.of(3, 5, 4));
        lenZeroToThreeChild.setRanDomain(List.of(6, 7));

        lensZeroToThree = new PyListNode<>(lenZeroToThreeChild);
        lensZeroToThree.setExDomain(List.of(0, 2));
        lensZeroToThree.setRanDomain(List.of(1, 3));
    ...
    private static Set<PyListObj<PyFloatObj>> genExpectedLenZeroToThree() {
        ...
        for (double i = 3; i < 6; i++) {
            ...
            for (double j = 3; j < 6; j++) {
                expected.add(new PyListObj<>(List.of(new PyFloatObj(i),
                        new PyFloatObj(j))));
            }
        }
        return expected;
        ...
        for (double i = 6; i < 8; i++) {
            for (double j = 6; j < 8; j++) {
                for (double k = 6; k < 8; k++) {
                    List<PyFloatObj> val = List.of(new PyFloatObj(i),
    ...
    private static void setUpNested() {
        ...
        inner.setExDomain(List.of(1, 2));
        inner.setRanDomain(List.of(3, 4));

        nestedBools = new PyListNode<>(inner);
        nestedBools.setExDomain(List.of(2, 0));
        nestedBools.setRanDomain(List.of(1, 3));
    ...
    private static Set<PyListObj<PyListObj<PyBoolObj>>> genExpectedNested() {
        ...
        List<PyBoolObj> val2 = List.of(new PyBoolObj(true), new PyBoolObj(true));
        ...
        for (PyListObj<PyBoolObj> innerObj : innerObjs) {
            for (PyListObj<PyBoolObj> innerObj2 : innerObjs) {
                List<PyListObj<PyBoolObj>> val = List.of(innerObj, innerObj2);
                expected.add(new PyListObj<>(val));
            }
        }
        return expected;
    ...
    private static Map<PyListObj<PyListObj<PyBoolObj>>, Double> genExpectedRandNested() {
        ...
        List<PyBoolObj> valLenThree = List.of(new PyBoolObj(false),
                new PyBoolObj(false), new PyBoolObj(false));
        List<PyBoolObj> valLenFour = List.of(new PyBoolObj(false),
        ...
        for (PyListObj<PyBoolObj> listObj : List.of(lenThree, lenFour)) {
            for (PyListObj<PyBoolObj> listObj2 : List.of(lenThree, lenFour)) {
                for (PyListObj<PyBoolObj> listObj3 : List.of(lenThree, lenFour)) {
                    List<PyListObj<PyBoolObj>> val = List.of(listObj, listObj2,
                            listObj3);
                    expected.put(new PyListObj<>(val), (1.0 / 16.0));
                }
            }
